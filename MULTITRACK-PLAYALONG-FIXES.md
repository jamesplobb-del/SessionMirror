...
> **STATUS (2026-08-04):** iOS Play All now bypasses multi-element WebKit audio entirely: all takes and uploaded backing audio are scheduled as `AVAudioPlayerNode`s on the native `MultitrackTransportEngine`; DOM videos are muted visual slaves. The Web-Audio path below remains the browser fallback and is hardened with a 2500ms ready budget, resume-to-running confirmation, grouped retry, and failure isolation so one rejected panel cannot release the others' route. Overdub offsets now come from the captured audio sample host clock plus the movie audio-track origin, rather than the lagging live `recordedDuration` estimate. The native click and references already share one sample clock. **Physical-device feel/measurement remains the final verification step.**
