    ...
    <div className="flex flex-col gap-4" aria-hidden>
      <div className="space-y-2">
        ...
        <div className="flex gap-2 overflow-hidden">
          <div className="h-8 w-24 shrink-0 animate-pulse rounded-lg bg-white/5" />
          <div className="h-8 w-28 shrink-0 animate-pulse rounded-lg bg-white/5" />
          <div className="h-8 w-24 shrink-0 animate-pulse rounded-lg bg-white/5" />
        </div>
      </div>
      ...
      <div className="flex items-start gap-4 overflow-hidden pb-2">
        {Array.from({ length: 4 }, (_, index) => (
          <div
            key={index}
            className="w-56 shrink-0 overflow-hidden rounded-2xl border border-white/10 bg-[#121212]"
          >
      ...
      {Array.from({ length: 4 }, (_, index) => (
        ...
        >
          <div className="flex items-start justify-between gap-4">
            ...
            <div className="h-7 w-12 shrink-0 animate-pulse rounded-full bg-white/10" />
          </div>
        </div>
      ))}
    </div>
