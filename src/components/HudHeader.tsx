  ...
  return (
    ...
    >
      ...
      <Pressable
        ...
        onClick={onOpenVault}
        haptic="light"
        className="pointer-events-auto flex max-w-[min(100%,16rem)] items-center justify-center rounded-full border-[0.5px] border-white/10 bg-black/40 px-3.5 py-1.5 shadow-lg backdrop-blur-2xl hover:border-white/15 hover:bg-black/55"
        aria-label={`Open vault — current session: ${sessionName}`}
      >
        <span className="ui-orient-spin flex max-w-full items-center gap-1.5">
          <FolderOpen className="h-3.5 w-3.5 shrink-0 text-white/85" />
          <span
