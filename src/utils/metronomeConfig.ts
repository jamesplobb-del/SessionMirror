...
export function studioTicksPerBar(
  meter: MetronomeMeter,
  subdivision: MetronomeSubdivision,
): number {
...
export function studioTickIntervalMs(
  bpm: number,
  meter: MetronomeMeter,
  subdivision: MetronomeSubdivision,
): number {
  const clampedBpm = Math.max(40, Math.min(240, bpm))
  const macroBeatSec = 60 / clampedBpm
  if (isCompoundMeter(meter) && subdivision === 'off') {
    return Math.round((macroBeatSec / 3) * 1000)
  }
  return Math.round((macroBeatSec / subdivisionsPerBeat(subdivision)) * 1000)
}

export function resolveStudioClickAccent(
  meter: MetronomeMeter,
  ...
  if (isCompoundMeter(meter) && subdivision === 'off') {
    tier = getCompoundClickTier(tickIndexInBar)
  } else {
    ...
    tier = tickInBeat === 0 ? getSimpleClickTier(beatIndex) : 'subdivision'
  }
  return tier === 'downbeat' || tier === 'macro'
}
