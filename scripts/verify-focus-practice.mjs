...
// Resume and atomic handoff preserve history, comparison notes, and item ownership.
await practice.updatePracticeItemState(project.id, { pendingIntention: 'Soft attack', loopStartSeconds: 2, loopEndSeconds: 6 })
const resumed = await practice.startPracticeSession({ projectId: project.id, focusArea: project.name, comparison: 'current-best', resumeSessionId: second.session.id })
assert.equal(resumed.session.id, second.session.id)
assert.equal(resumed.state.pendingIntention, 'Soft attack')
await Promise.all([
  practice.updatePracticeItemState(project.id, { comparison: 'previous-take' }),
  practice.updatePracticeItemState(project.id, { pendingIntention: 'Even release' }),
])
const preserved = await practice.getPracticeItemState(project.id)
assert.equal(preserved.comparison, 'previous-take')
assert.equal(preserved.pendingIntention, 'Even release')
assert.equal(preserved.loopStartSeconds, 2)
assert.equal(preserved.lastSessionId, second.session.id, 'Partial updates must not overwrite the sitting binding')
await assert.rejects(practice.startPracticeSession({ projectId: 'missing-project', focusArea: 'Missing', comparison: 'current-best', replaceSessionId: second.session.id }))
assert.equal((await practice.listPracticeSessions(project.id)).find(row => row.id === second.session.id).endedAt, null, 'Failed start rolls back closing the previous sitting')
const routineSitting = await practice.startPracticeSession({ projectId: other.id, focusArea: other.name, comparison: 'reference-track', routineId: 'routine-a', routineStepId: 'step-a', replaceSessionId: second.session.id })
assert.equal(routineSitting.session.routineId, 'routine-a')
assert.ok((await practice.listPracticeSessions(project.id)).find(row => row.id === second.session.id).endedAt)
const recovered = await practice.startPracticeSession({ projectId: other.id, focusArea: other.name, comparison: 'reference-track', routineId: 'routine-a', routineStepId: 'step-a' })
assert.equal(recovered.session.id, routineSitting.session.id, 'SQLite recovers a sitting after a missing local checkpoint')
assert.equal((await practice.findRoutinePracticeProject('routine-a', 'step-a', null)).id, other.id)
assert.equal(await practice.findRoutinePracticeProject('routine-b', 'step-a', null), null, 'Identical exercise names/step IDs in another routine cannot share implicitly')
const wrongContext = await practice.startPracticeSession({ projectId: other.id, focusArea: other.name, comparison: 'current-best', routineId: 'routine-b', routineStepId: 'step-a', resumeSessionId: routineSitting.session.id })
assert.notEqual(wrongContext.session.id, routineSitting.session.id, 'Explicit resume IDs are validated against the routine and item')
const closedTime = (await practice.listPracticeSessions(project.id)).find(row => row.id === second.session.id).endedAt
await practice.endPracticeSession(second.session.id)
assert.equal((await practice.listPracticeSessions(project.id)).find(row => row.id === second.session.id).endedAt, closedTime, 'Ending is idempotent')
const summary = await practice.summarizePracticeSessions([first.session.id, second.session.id, second.session.id, routineSitting.session.id])
assert.equal(summary.attempts, 2, 'Recap counts saved takes, deduplicates sessions, includes zero-take exercises honestly')
assert.equal(summary.recordedSeconds, 25)
assert.equal((await practice.summarizePracticeSessions([])).attempts, 0)
await migrateVaultSchema(adapter)
assert.equal((await practice.listPracticeSessions(other.id)).length, 2, 'Repeated migration preserves session rows')
...
sqlite.run(CREATE_SCHEMA_SQL.replace('  routine_id TEXT,\n  routine_step_id TEXT,\n', ''))
const oldProject = await vault.createProject('Existing install')
sqlite.run("INSERT INTO practice_sessions (id, project_id, started_at, focus_area) VALUES ('legacy-sitting', ?, 1000, 'Old focus')", [oldProject.id])
await migrateVaultSchema(adapter)
await migrateVaultSchema(adapter)
const migratedSession = (await practice.listPracticeSessions(oldProject.id))[0]
assert.equal(migratedSession.id, 'legacy-sitting')
assert.equal(migratedSession.routineId, null)
assert.equal(migratedSession.focusArea, 'Old focus')
sqlite.close()
